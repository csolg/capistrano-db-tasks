    ...
    def initialize(cap_instance)
      ...
      @cap.within @cap.current_path do
        @cap.with rails_env: @cap.fetch(:rails_env) do
          config_content = @cap.capture(:cat, "config/database.yml", '2>/dev/null')
          full_config = YAML.load(config_content).each_with_object({}) { |(k, v), h| h[k.to_s] = v }
          @config = full_config[fetch(:rails_env)]
        end
      end
    end
